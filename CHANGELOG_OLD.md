...
This file keeps pre-public-release changelog entries that are no longer listed
in `io-package.json` `common.news`, because the ioBroker repository checker
expects `common.news` to reference only versions that exist on npm.
